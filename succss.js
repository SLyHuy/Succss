						...
						casperInstance.eachThen(viewports, function(response) {
							...
							var v = response.data;
								...
								try {
									...
									self.echo('\n-----------------------------------------------------------------------------');
									self.echo('Capturing "' + capture.page.name + '" > "' + capture.name + '" screenshot with ' + v + ' viewport:', 'INFO');
									self.echo('> Selector is: "' + JSON.stringify(capture.selector) + '"', 'PARAMETER');
	...
	self.takeScreenshot = function(captureState) {
		...
		casperInstance.then(function() {
			...
			if (captureState.before) {
				...
				self.echo('> Run before function', 'PARAMETER');
				captureState.before.call(casperInstance);
			}
		...
		casperInstance.then(function(){
			casperInstance.waitForSelector(captureState.selector, function() {
				self.echo('> Wait for selector: "' + JSON.stringify(captureState.selector) + '" successfull', 'PARAMETER');
				
			}, function() {
				self.echo('> Selector "' + JSON.stringify(captureState.selector) + '" was not found anywhere on the page. Timeout!', 'ERROR');
			});
		}, 0);
		...
		casperInstance.then(function() {
			try {
				...
				self.echo('> Saving: "' + captureState.name + '"', 'PARAMETER');
				takeScreenshotOke = true;
			}
			catch (err) {
				self.echo(err, 'ERROR');
				self.catchErrors(err);
				takeScreenshotOke = false;
			}
		});

		casperInstance.then(function(){
			if (takeScreenshotOke){
				self.echo('>>> Saved image successfully in: ' + captureState.filePath, 'INFO');
			}

			if (captureState.sAfter){
				self.echo('> Run sAfter function', 'PARAMETER');
				captureState.sAfter.call(casperInstance);
			}
			self.echo('-----------------------------------------------------------------------------');
		});
	}
